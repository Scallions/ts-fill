...
def compare_one_gap():
    ...
    fillers = [fill.PolyFiller, fill.SLinearFiller, fill.GANFiller]
...
def compare_mul_gap():
    ...
    fillers = [fill.PolyFiller, fill.SLinearFiller]
    for filler in fillers:
        ts4 = filler().fill(ts3)
        plt.plot(ts4.loc[gidx[:gapsize], cidx[0]], 'o', label=filler.name,
            markersize=3)
        res = tool.fill_res(ts2, ts4, gidx)
        logger.debug(filler.name)
        logger.debug(res)
