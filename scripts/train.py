...
class TsDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        return self.data.iloc[index:index + length].values, self.data.iloc[
            index + length].values
...
def train(tss, net, loss):
    ...
    opt = torch.optim.Adam(net.parameters(), lr=1e-05, weight_decay=0.1)
    epochs = 400
    checkpoint = True
    for epoch in range(epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            if i % 30 == 29:
                logger.info(
                    f'Epoch: {epoch}, Batch: {i}, Loss: {l.data.item()}')
        logger.info(f'Epoch: {epoch}, Loss: {lm}')
        if checkpoint and epoch % 2 == 1:
            torch.save({'epoch': epoch, 'model_state_dict': net.state_dict(
                ), 'opt_state_dict': opt.state_dict(), 'loss': l},
...
if __name__ == '__main__':
    logger.add('log/train_gln_{time}.log', rotation='500MB', encoding=
