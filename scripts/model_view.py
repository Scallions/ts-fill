...
def testm(tss, net):
    for ts in tss:
        ...
        tsg, gidx, cidx = ts.make_gap(100, cache_size=300, per=100 / 1024,
            c_i=False)
...
def test(tss, net):
    for ts in tss:
        ...
        tsg, gidx = ts.make_gap(200, cache_size=30, per=0.03)
...
if __name__ == '__main__':
    logger.add('log/view_{time}.log', rotation='500MB', encoding='utf-8',
        enqueue=True, compression='zip', retention='10 days', level='INFO')
    tss = load_mdata()
